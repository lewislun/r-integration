...
describe('validator callMethodAsync()', function() {
	
	it('should return 10 if calling the method x with parameter 5', async function() {
		const result = await validator.callMethodAsync('test/RScripts/test_method.R', 'x', [5])
		expect(result).to.have.members([10])
	})

	it('should return 10 if calling the method x with object parameter 5', async function() {
		const result = await validator.callMethodAsync('test/RScripts/test_method.R', 'x', {data: 5})
		expect(result).to.have.members([10])
	})

	it('should return the max between 10 and 20', async function() {
		const result = await validator.callMethodAsync('test/RScripts/test_method_multiple_params.R', 'max', [10, 20])
		expect(result).to.have.members([20])
	})

	it('should return the max between 10 and 20 with object parameters', async function() {
		const result = await validator.callMethod('test/RScripts/test_method_multiple_params.R', 'newMax',{x:10, y:20})
		expect(result).to.have.members([20])
	})
